...
class TestPortfolioViewTestCase(TestSetup):
    ...
    def test_portfolio_view_url_by_name(self):
        response = self.client.get(self.portfolio_url)
        self.assertEquals(response.status_code, 200)
    ...
    def test_portfolio_page_contains_correct_html(self):
        response = self.client.get(self.portfolio_url)
        self.assertContains(response, """<h1>my <span>portfolio</span></h1>""")
        # self.assertContains(response, """<h1>my <span>portfolio</span></h1>""")
        # self.assertContains(response, """more about me""")

    def test_portfolio_page_does_not_contain_incorrect_html(self):
        response = self.client.get(self.portfolio_url)
